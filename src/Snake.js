...
const Snake = () => {
  ...
  const generateCells = () => {
    ...
    for (let i = 1; i <= maxY; i++) {
      for (let j = 1; j <= maxX; j++) {
        let isHead = false, isFood = false, isTail = false;
  ...
  return (<>
    <div class="bar">
      ...
      <button style={{ float: 'right', marginRight: 300 }} onMouseDown={() => setPaused(!paused)}>Pause</button>
    </div>
    <div className="grid popup">
      <span class="popuptext" id="myPopup">Game over. Press Space to restart.</span>
      {head && generateCells()}
    </div>
  </>
  )
